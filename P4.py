...
# ---
# 
# * Estudiante: **Alexander Calderón Torres**
# * Carné: **B61325**
# * Grupo: **1**
# 
# ---
...
# 
# #### 4.1.1. - Creación y adaptación de las funciones a utilizar

# In[9]:


### Ajuste y redefinición de modulación,
### para que dos moduladores trabajen con dos bits a la vez.
...
def modulador_I(bits, fc, mpp):
    ...
    # 2. Construyendo un período de la señal portadora c(t) con coseno
    Tc = 1 / fc  # periodo [s]
    t_periodo = np.linspace(0, Tc, mpp)  # mpp: muestras por período
    ...
    j=0
    k=1
    while k in range(0,len(bits)):
        # En esta lógica no fue necesario trabajar estrictamente con números binarios
        primeros_bits = int(str(bits[j])+str(bits[k]))
        if primeros_bits == 11:
            senal_Tx[i*mpp : (i+1)*mpp] = portadora *  1
        elif (primeros_bits == 1):
            senal_Tx[i*mpp : (i+1)*mpp] = portadora * -1
        elif (primeros_bits == 10):
            senal_Tx[i*mpp : (i+1)*mpp] = portadora *  3
        elif (primeros_bits == 0):
            senal_Tx[i*mpp : (i+1)*mpp] = portadora * -3
        i+=1
        k+=2
        j=k-1
    ...
    return senal_Tx, P_senal_Tx, portadora
...
def modulador_Q(bits, fc, mpp):
    ...
    while k in range(0,len(bits)):
        # En esta lógica no fue necesario trabajar estrictamente con números binarios
        ultimos_bits = int(str(bits[j])+str(bits[k]))
        if ultimos_bits == 11:
            senal_Tx[i*mpp : (i+1)*mpp] = portadora * -1
        elif (ultimos_bits == 1):
            senal_Tx[i*mpp : (i+1)*mpp] = portadora *  1
        elif (ultimos_bits == 10):
            senal_Tx[i*mpp : (i+1)*mpp] = portadora * -3
        elif (ultimos_bits == 0):
            senal_Tx[i*mpp : (i+1)*mpp] = portadora *  3
        i+=1
...
def combinador(senal_Tx_I, P_senal_Tx_I, senal_Tx_Q, P_senal_Tx_Q):
    '''
    Combina las señales en una sola, la que se va a transmitir finalmente.
    Estas señales de entrada están desfasadas en 90 grados.
    '''
    senal_Tx_I_list = list(senal_Tx_I)
    senal_Tx_Q_list = list(senal_Tx_Q)
    senal_Tx = []
    
    for i in range(0,len(senal_Tx_I_list),2):
        k=i
        senal_Tx.append(senal_Tx_I_list[k])
        senal_Tx.append(senal_Tx_I_list[k+1])
        k=i
        senal_Tx.append(senal_Tx_Q_list[k])
        senal_Tx.append(senal_Tx_Q_list[k+1])
        
    return np.array(senal_Tx), P_senal_Tx_I+P_senal_Tx_Q

def demodulador_16_QAM(senal_Rx, portadora_I, portadora_Q, mpp):
    '''Un método que simula un bloque demodulador
    ...
    :param senal_Rx: La señal recibida del canal
    :param portadora_I: La primera onda portadora
    :param portadora_Q: La segunda onda portadora
    :param mpp: Número de muestras por periodo
    :return: Los bits de la señal demodulada
    '''
    # Cantidad de muestras en senal_Rx
    ...
    for i in range(0,N-1,4):
        ...
        if Ep_I > 2.5*Es_I:
            ...
            bits_Rx[i] = 0
            bits_Rx[i+1] = 1
        elif Ep_I > -0.5*Es_I and Ep_I < 0.5*Es_I:
            bits_Rx[i] = 0
            bits_Rx[i+1] = 0
        ...
        if Ep_Q >= 2.5*Es_Q:
            ...
            bits_Rx[i+2] = 1
            bits_Rx[i+3] = 1
        elif Ep_Q > -0.5*Es_Q and Ep_Q < 0.5*Es_Q:
            bits_Rx[i+2] = 0
            bits_Rx[i+3] = 0
    return bits_Rx.astype(int), senal_demodulada
...
# Parámetros
fc = 5000  # frecuencia de la portadora
mpp = 20   # muestras por período de la portadora
SNR = 5   # relación señal-a-ruido del canal
...
# 2. Codificar los pixeles de la imagen
bits_Tx = rgb_a_bit(imagen_Tx) # Estos bits están en un array, no en un número binario

# 3. Modular la cadena de bits usando el esquema 16-QAM
## Se dividen los bits para pasarlos por ambos moduladores
...
while k in range(0,len(bits_Tx)):
    bits_Tx_I.append(bits_Tx[j-1])
    bits_Tx_I.append(bits_Tx[j])
    bits_Tx_Q.append(bits_Tx[k-1])
    bits_Tx_Q.append(bits_Tx[k])

    j+=4
    k+=4
    
senal_Tx_I, P_senal_Tx_I, portadora_I = modulador_I(bits_Tx_I, fc, mpp)
senal_Tx_Q, P_senal_Tx_Q, portadora_Q = modulador_Q(bits_Tx_Q, fc, mpp)

senal_Tx, Pm = combinador(senal_Tx_I, P_senal_Tx_I, senal_Tx_Q, P_senal_Tx_Q) 

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador_16_QAM(senal_Rx, portadora_I, portadora_Q, mpp)
